(ns anduril_lattice.main
  "Kotodama WASM entrypoint for the Anduril_lattice clean-room actor (L4) — Clojure port.
  ...
  facts (`anduril_lattice.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "anduril_lattice")
(def tier "L4")
...
(def entity-specs
  [{:entity "Case"      :plural "cases"      :id-prefix "andurill_cas"
    :fields [:number :jurisdiction :status :filedAt] :required [:number :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Party"     :plural "parties"    :id-prefix "andurill_par"
    :fields [:caseId :role :name]            :required [:role :name]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Document"  :plural "documents"  :id-prefix "andurill_doc"
    :fields [:caseId :type :contentRef]      :required [:type]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Incident"  :plural "incidents"  :id-prefix "andurill_inc"
    :fields [:type :jurisdiction :occurredAt] :required [:type :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Watchlist" :plural "watchlists" :id-prefix "andurill_wat"
    :fields [:name :program :entries]        :required [:name :program]
    :coerce {:entries :int}          :refs {}}
   {:entity "Sanction"  :plural "sanctions"  :id-prefix "andurill_san"
...
(defn emit-facts
  "EAVT facts for one record: {\"anduril_lattice.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "anduril_lattice-compat" :tier tier :entities entities} 200])
...
(defn start! [] :anduril_lattice-compat/ready)
